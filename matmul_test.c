...
int main(void)
{
        ...
        ///// COO format data /////
        __fp16 coo_value[8] = {1.03, 5.2, 7, 3.1, 10, 2, 9, 7};
        uint8_t coo_row[8] = {1, 2, 6, 8, 10, 11, 14, 15};
        uint8_t coo_col[8] = {0, 3, 5, 7, 11, 13, 14, 15};
        ...
        printf("\nbefore memcpy check onchip_ptr data \n");
        ...
        memcpy((void *)mat_vector_ptr, mat_vector, 8*2);
        memcpy((void *)row_ptr_ptr, row_ptr, 32);
        memcpy((void *)col_idx_ptr, col_idx, 4);
        clock_gettime(CLOCK_MONOTONIC, &end2);

        printf("\nafter memcpy check onchip_ptr data \n");
        for(i=0; i<512; i++)
        {
                printf("%d  ",*(onchip_ptr+i));
        }
        ...
        *(onchip_ptr) = 1;
        ...
        for(i=0; i<8; i++)
        {
                printf("%d  ", *(in_vector_ptr + i));
        }
        printf("\nmat_vector:\n");

        for(i=0; i<128; i++)
        {
                printf("%d  ", *(mat_vector_ptr + i));
        }

        printf("\nrow_ptr:\n");
        for(i=0; i<8; i++)
        {
                printf("%d  ", *(row_ptr_ptr + i));
        }

        printf("\ncol_idx:\n");
        for(i=0; i<8; i++)
        {
                printf("%d  ", *(col_idx_ptr + i));
        }
